...
BOTO3_CONFIG = boto3.session.Config(
    ...
    retries={"max_attempts": 3, "mode": "adaptive", "total_max_attempts": 3},
        ...
        def wrapper(*args, **kwargs):
            ...
            with _cache_lock:
                ...
                if len(_global_cache) > 1000:
                    expired_keys = [
                        k
                        for k, (_, t) in _global_cache.items()
...
openai_client = ChatOpenAI(api_key=os.environ["OPEN_AI_KEY"], logger=logger)

# OpenAI embeddings with caching
embedding_client = OpenAIEmbeddings(api_key=os.environ["OPEN_AI_KEY"], logger=logger)
    ...
    def get_agent_details(self, agent_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if "Item" in response:
                ...
                needs_refresh = (
                    not item.get("biography")
                    or item.get("biography") == ""
                    or self._is_data_stale(item)
                )

                if needs_refresh:
                    logger.info(
                        f"Data needs refresh for {agent_id}, fetching from canister"
                    )
            ...
            try:
                ...
                if result and len(result) > 0:
                    ...
                    if isinstance(wizard_data, str):
                        logger.error(
                            f"Unexpected string response from canister: {wizard_data}"
                        )
                        return None
                    ...
                    if isinstance(wizard_data, list):
                        if len(wizard_data) == 0:
                            logger.warning(
                                f"Empty list returned for agent_id: {agent_id}"
                            )
                            return None
                        wizard_data = wizard_data[0]
                        if wizard_data is None:
                            logger.warning(
                                f"None value in list for agent_id: {agent_id}"
                            )
                            return None
                    ...
                    if isinstance(wizard_data, dict):
                        ...
                        logger.debug(
                            f"Successfully extracted agent data for {agent_id}"
                        )
                        return agent_data
                    else:
                        logger.error(
                            f"Unexpected data type from canister: {type(wizard_data)}"
                        )
                        return None

                logger.warning(
                    f"Empty or invalid response from canister for agent_id: {agent_id}"
                )
                return None

            except Exception as e:
                logger.error(
                    f"Error fetching from canister (attempt {attempt + 1}): {str(e)}"
                )
                if attempt < max_retries:
                    time.sleep(retry_delay * (attempt + 1))
                    continue
...
class ChatHistoryHandler:
    ...
    def store_chat_message(self, user_id: str, agent_id: str, role: str, content: str):
        ...
        try:
            ...
            self.table.put_item(Item=item)
            self.logger.debug(
                f"Successfully stored {role} message for {user_id}#{agent_id}"
            )
    ...
    def store_chat_messages_batch(
        self, user_id: str, agent_id: str, messages: List[Dict]
    ):
        ...
        try:
            ...
            self.logger.debug(
                f"Successfully stored {len(messages)} messages for {user_agent_id}"
            )
...
agent_data_handler = AgentDataHandler(
    os.environ["AGENT_DETAILS_TABLE"], dynamodb_client, logger
)
history_handler = ChatHistoryHandler(
    os.environ["CHAT_HISTORY_TABLE"], dynamodb_client, logger
)
...
class ChatProcessor:
    ...
    def __init__(self):
        self.executor = ThreadPoolExecutor(
            max_workers=12, thread_name_prefix="ChatProcessor"  # Increased workers
        )
        self.embedding_cache = {}
        self.prompt_cache = {}

    def process_chat_sync(
        self, user_id: str, agent_id: str, query_text: str, idempotency_key: str
    ) -> Dict:
        """Ultra-optimized chat processing with maximum parallelization and smart caching"""
        start_time = time.time()

        logger.info(
            f"Starting ultra-optimized chat processing for user {user_id}, agent {agent_id}"
        )
        ...
        try:
            ...
            agent_details = futures["agent_details"].result(
                timeout=3.0
            )  # Increased timeout
            ...
            if embeddings:
                ...
                logger.warning(
                    "No embeddings available, proceeding without vector search"
                )
        ...
        try:
            ...
            chat_history = futures["chat_history"].result(
                timeout=2.0
            )  # Increased timeout
            ...
            if "vector_search" in futures:
                vector_results = futures["vector_search"].result(
                    timeout=4.0
                )  # Increased timeout
            else:
        ...
        try:
            ...
            if (
                not ai_response
                or ai_response.strip() == ""
                or ai_response.lower() == "none"
            ):
            ...
            return {"statusCode": 200, "body": {"response": ai_response}}
    ...
    def _get_embeddings_with_retry(
        self, query_text: str, query_hash: str
    ) -> Optional[List[float]]:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if embeddings and len(embeddings) > 0:
                    logger.info(
                        f"Generated embeddings successfully, length: {len(embeddings)}"
                    )
                    return embeddings
            except Exception as e:
                if attempt < max_retries:
            ...
            if result and len(result) > 0:
                ...
                if isinstance(search_data, dict):
                    ...
                    if "ok" in search_data:
                        ...
                        combined_text = "\n".join(search_data["ok"])
                        logger.info(
                            f"Successfully extracted {len(search_data['ok'])} text items from 'ok' field"
                        )
                    ...
                    # Handle the nested structure: {'type': 'rec_0', 'value': {...}}
                        ...
                        if isinstance(value_data, dict):
                            ...
                            for key, content in value_data.items():
                                logger.info(
                                    f"Processing key: {key}, content type: {type(content)}"
                                )
                            ...
                            if texts:
                                combined_text = "\n".join(texts)
                                logger.info(
                                    f"Successfully extracted {len(texts)} text items, total length: {len(combined_text)}"
                                )
                                return combined_text
                            else:
                            ...
                            logger.warning(
                                f"value_data is not a dict: {type(value_data)}"
                            )
                            return "no content"
                    ...
                    # Fallback: try to extract any string content from the dict - same as original
                        ...
                        if texts:
                            combined_text = "\n".join(texts)
                            logger.info(
                                f"Fallback extraction found {len(texts)} text items"
                            )
                            return combined_text
                        else:
    ...
    def _store_chat_async_optimized(
        self, user_id: str, agent_id: str, query: str, response: str
    ):
        ...
        try:
            ...
            self.logger.debug(
                f"Successfully stored chat messages for {user_id}#{agent_id}"
            )
...
Helpful Answer:"""
        ...
        return {"system_message": base_template, "user_message": query_prompt}
    ...
    def _process_history_optimized(self, history_items: List[Dict]) -> str:
        ...
        recent_items = history_items[
            -10:
        ]  # Only last 10 messages for better performance
    ...
    def _get_ai_response_sync(self, prompt_data: Dict, idempotency_key: str) -> str:
        ...
        try:
            ...
            full_prompt = (
                f"{prompt_data['system_message']}\n\n{prompt_data['user_message']}"
            )
            logger.info(
                f"Sending prompt to OpenAI: {full_prompt[:200]}..."
            )  # Log first 200 chars
            ...
            if response is None:
                ...
                if content:
                    ...
                    if not content_str or content_str.lower() in ["none", "null", ""]:
                        logger.error(
                            f"OpenAI response dict has invalid content: '{content_str}'"
                        )
                        return "I apologize, but I couldn't generate a response. Please try again."
                    return content_str
                else:
                    logger.error(
                        f"OpenAI response dict has no usable content: {response}"
                    )
                    return "I apologize, but I couldn't generate a response. Please try again."
            else:
                ...
                if not response_str or response_str.lower() in ["none", "null", ""]:
                    logger.error(
                        f"OpenAI response converted to invalid string: '{response_str}'"
                    )
                    return "I apologize, but I couldn't generate a response. Please try again."
                return response_str
...
def chat():
    ...
    try:
        ...
        result = chat_processor.process_chat_sync(
            user_id, agent_id, query_text, idempotency_key
        )
    ...
    try:
        ...
        if not user_id:
            return Response(
                ...
                content_type=content_types.APPLICATION_JSON,
                body=json.dumps(
                    {"error": "user_id is required in query parameters or request body"}
                ),
            )
        ...
        with _cache_lock:
            ...
            keys_to_remove = [
                k
                for k in _global_cache.keys()
                if f"{user_id}#{agent_id}" in k or f"get_chat_history" in k
            ]
